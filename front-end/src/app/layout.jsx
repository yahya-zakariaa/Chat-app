...
export default function RootLayout({ children }) {
  ...
  return (
    <html lang="en">
      <body
        className={`${geistSans.variable} ${geistMono.variable}  flex flex-col relative justify-start gap-2 md:justify-between p-2 max-h-[100vh] h-screen overflow-hidden bg-[#0a0a0a]`}
      >
        ...
        <nav className=" w-[100%] md:px-5 px-3  rounded-xl mx-auto h-[10%] flex justify-between items-center bg-gray-800">
        ...
        {isToggled && windowWidth <= 500 && (
          <div
            onClick={() => setIsToggled(false)}
            className="md:hidden fixed top-0 left-0 w-screen h-screen bg-black bg-opacity-60 z-[9999]"
          >
            {currentComponent}
          </div>
        )}
        ...
        <main className="flex md:flex-row flex-col-reverse gap-2 justify-between items-start md:items-end w-full md:h-[88%] h-[88%] ">
          {pagesWithSidebar.includes(pathname) && (
            <div
              onClick={() => null}
              className="sidebar rounded-xl overflow-hidden  flex flex-row md:flex-col md:py-3 justify-between items-center md:h-[100%] lg:w-[5%] md:w-[7%] w-[100%] left-[50%] translate-x-[-50%] md:translate-x-0 h-[12%]     shadow-3xl  md:bg-[#0e1428] bg-[#0b1942c6] md:backdrop-blur-0 backdrop-blur-md md:relative relative md:top-0 md:left-0 z-[999]"
            >
