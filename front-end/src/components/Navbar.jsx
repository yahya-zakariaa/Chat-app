    ...
    <nav className=" w-[100%] md:px-4 px-3  rounded-xl mx-auto  min-h-[10%]  h-[10%] flex justify-between items-center bg-[#1a1a1a]">
      <div className="userProfile flex items-center">
        {user && !isCheckingAuth ? (
          ...
          <div
            alt="user avatar skeleton"
            className="rounded-full skeleton-loader min-w-[45px] min-h-[45px] h-[45px] w-[45px] aspect-square object-cover"
            width={45}
            height={45}
          />
        )}
        ...
        {!isCheckingAuth && user ? (
          <>
            ...
            <li aria-label="notifications ">
              <button
                aria-label="notifications flex items-center"
                onClick={() => {
          ...
          Array(2)
            ...
            .map((_, index) => (
              <div
                key={index}
                className="rounded-xl skeleton-loader w-[30px] h-[30px] min-w-[30px] min-h-[30px] "
              ></div>
            ))
        )}
      </ul>
